#!/usr/bin/env python3
"""Interactive setup wizard for the AI Agent Harness.
...
def banner() -> None:
    print("=" * 60)
    print("  AI Agent Harness — Setup Wizard")
    print("=" * 60)
    print()
    print("This wizard will configure the AI Agent Harness")
...
def generate(platform: dict, store: dict, level: int, project: dict) -> None:
    ...
    if level >= 4:
        ...
        # Generate project consistency checker
        tests_dir = project_dir / "tests"
        tests_dir.mkdir(parents=True, exist_ok=True)
        _generate_consistency_checker(tests_dir, platform["hooks_dir"])
        print(f"  [OK] Consistency checker -> {tests_dir}/consistency_check.py")

        # Copy sensitive patterns example
        src_patterns = REPO_DIR / "tests" / "sensitive-patterns.example.txt"
        if src_patterns.exists():
            shutil.copy2(src_patterns, tests_dir / "sensitive-patterns.example.txt")
            print(f"  [OK] Sensitive patterns template -> {tests_dir}/sensitive-patterns.example.txt")
    ...
    if level >= 4:
        ...
        print(f"  python3 tests/consistency_check.py --verbose")
...
"""
    if path.exists():
        ...
        if "AI Agent Harness" not in content:
            content += "\n" + startup_text
...
def _generate_consistency_checker(tests_dir: Path, hooks_rel: str) -> None:
    """Generate a project-specific structural consistency checker."""
    checker = tests_dir / "consistency_check.py"
    checker.write_text(f'''#!/usr/bin/env python3
"""Structural consistency checker for your project.

Verifies hooks, config, rules, and sensitive data are in sync.
Generated by the AI Agent Harness setup wizard.
...
def check_hook_syntax():
    ...
    section("1. HOOK SYNTAX")
    if not HOOKS_DIR.exists():
        warn(f"Hooks directory not found: {{HOOKS_DIR}}")
        return
...
def check_hooks_exist_in_settings():
    """2. Hooks referenced in settings.json exist on disk."""
    section("2. HOOKS IN SETTINGS")
    settings_path = PROJECT / "{hooks_rel.split('/')[0]}" / "settings.json"
    if not settings_path.exists():
        warn(f"Settings not found: {{settings_path}}")
        return
    try:
            ...
            for hook in entry.get("hooks", []):
                ...
                if script:
                    ...
                    if script_path.exists():
                        ok(f"{{event}}: {{os.path.basename(script)}} exists")
                    else:
                        fail(f"{{event}}: {{script}} NOT found")


def check_config_valid():
    """3. Startup config parses correctly."""
    section("3. CONFIG VALIDITY")
    config_path = PROJECT / "startup-config.yaml"
    if not config_path.exists():
        warn("No startup-config.yaml found")
        return
    try:
        import yaml
        data = yaml.safe_load(config_path.read_text())
        tiers = data.get("tiers", {{}})
        ok(f"startup-config.yaml: {{len(tiers.get(\'tier1\', []))}} tier1, "
           f"{{len(tiers.get(\'tier2\', []))}} tier2")
    except ImportError:
        warn("PyYAML not installed — skipping config validation")
    except Exception as e:
        fail(f"startup-config.yaml: {{e}}")


def check_rules_sources_exist():
    """4. Rule sources referenced in config exist on disk."""
    section("4. RULE SOURCES")
    config_path = PROJECT / "startup-config.yaml"
    if not config_path.exists():
        warn("No startup-config.yaml — skipping")
        return
    try:
        import yaml
        data = yaml.safe_load(config_path.read_text())
    ...
    for tier_name in ("tier1", "tier2"):
        for entry in data.get("tiers", {{}}).get(tier_name, []):
            source = entry.get("source")
            if not source or entry.get("type") in ("checks", "generated"):
                continue
            source_path = PROJECT / source
            if source_path.exists():
                ok(f"{{entry.get(\'name\', source)}}: {{source}}")
            else:
                fail(f"{{entry.get(\'name\', source)}}: {{source}} NOT found")


def check_sensitive_data():
    """5. No sensitive data in tracked files."""
    section("5. SENSITIVE DATA SCAN")
    patterns_file = PROJECT / "tests" / "sensitive-patterns.txt"
    if not patterns_file.exists():
        warn("No tests/sensitive-patterns.txt — copy from "
             "sensitive-patterns.example.txt and customize")
        return
    patterns = [
    ...
    if not patterns:
        warn("sensitive-patterns.txt is empty")
        return
    ...
    for ext in ("*.py", "*.md", "*.yaml", "*.yml", "*.json"):
        for f in PROJECT.rglob(ext):
            if "__pycache__" in str(f) or ".git" in f.parts:
                continue
            if f.name in ("consistency_check.py", "sensitive-patterns.txt"):
                continue
            try:
                for i, line in enumerate(f.read_text().splitlines(), 1):
                    if regex.search(line):
                        found.append((f.relative_to(PROJECT), i,
                                      line.strip()[:80]))
            except Exception:
                pass
    if not found:
...
def main():
    ...
    print("=" * 50)
    check_hook_syntax()
    check_hooks_exist_in_settings()
    check_config_valid()
    check_rules_sources_exist()
